/*
Fixed Evaluator Module - Expression Evaluation Engine
=====================================================
...
Bug Fixes Applied:
1. Added missing log10 case implementation
2. Corrected error message for ln (was incorrectly saying "log10")
3. Added log2 to the switch case for proper handling
4. Improved overflow detection for pow function and ^ operator
5. Added NaN detection for invalid operations
6. Better domain validation across all functions
	...
	case parser.NODE_OPERATOR:
		...
		case "^":
			...
			if math.IsInf(result, 0) {
				return 0, fmt.Errorf("exponentiation overflow")
			}
			if math.IsNaN(result) {
				return 0, fmt.Errorf("exponentiation produced invalid result")
			}
			return result, nil
		default:
      ...
      return printResult,nil
		...
		case "pow", "max", "min", "atan2", "mod":
			...
			case "pow":
				...
				if math.IsNaN(result) {
					return 0, fmt.Errorf("pow(%g,%g) invalid result", arg1, arg2)
				}
				return result, nil
			case "max":
